...
static struct regulator_init_data ldo5_data 		 
	= ADJ_REGULATOR_INIT(ldo5,1250, 3300, 0, 1); // 2850
static struct regulator_init_data ldo6_data 		 
	= ADJ_REGULATOR_INIT(ldo6,1250, 3300, 0, 1); // 2850  V-3V3 USB
...
static struct regulator_init_data ldo9_data 		 
	= ADJ_REGULATOR_INIT(ldo9,1250, 3300, 0, 1); // 2850
...
static struct fixed_voltage_config ldo_tps2051B_cfg
	= FIXED_REGULATOR_CONFIG(ldo_tps2051B  , 5000, GTABLET_ENABLE_VDD_VID	, 1,1, 500000, 0, ldo_tps2051B_data);
...
static struct tps6586x_rtc_platform_data gtablet_rtc_data = {
...
static struct tps6586x_subdev_info tps_devs[] = {
	...
	TPS_GPIO_FIX_REG(2, &ldo_tps72012_cfg),*/
	{
		.id		= -1,
		.name		= "tps6586x-rtc",
		.platform_data	= &gtablet_rtc_data,
	},
};
...
static struct i2c_board_info __initdata gtablet_regulators[] = {
	{
...
static struct platform_device gtablet_ldo_tps2051B_reg_device = 
	GPIO_FIXED_REG(3,ldo_tps2051B_cfg); /* id is 3, because 0-2 are already used in the PMU gpio controlled fixed regulators */

static struct platform_device gtablet_vdd_aon_reg_device = 
{
...
static struct nvec_power_platform_data nvec_power_pdata = {
	.low_batt_irq = TEGRA_GPIO_TO_IRQ(GTABLET_LOW_BATT),	/* If there is a low battery IRQ */
	.in_s3_state_gpio = GTABLET_IN_S3,						/* Gpio pin used to flag that system is suspended */
...
static struct nvec_platform_data nvec_mfd_platform_data = {
	.i2c_addr	= GTABLET_NVEC_I2C_ADDR,
	.gpio		= GTABLET_NVEC_REQ,
	.irq		= INT_I2C3,
...
static struct platform_device gtablet_nvec_mfd = {
	.name = "nvec",
	.dev = {
...
static struct platform_device gtablet_bq24610_device= {
	.name = "bq24610",
	.dev = {
...
static void gtablet_power_off(void)
{
...
static struct platform_device *gtablet_power_devices[] __initdata = {
	//&gtablet_ldo_tps2051B_reg_device,
	//&gtablet_vdd_aon_reg_device,
	...
	//&gtablet_nvec_mfd,
	&tegra_rtc_device,
	//&gtablet_bq24610_device,
};

/* Init power management unit of Tegra2 */
int __init gtablet_power_register_devices(void)
{
	int err;
	...
	err = i2c_register_board_info(4, gtablet_regulators, 1);
	if (err < 0) 
		pr_warning("Unable to initialize regulator\n");

	/* register the poweroff callback */
	pm_power_off = gtablet_power_off;		
	...
	//return 0;
	return platform_add_devices(gtablet_power_devices, ARRAY_SIZE(gtablet_power_devices));
}
