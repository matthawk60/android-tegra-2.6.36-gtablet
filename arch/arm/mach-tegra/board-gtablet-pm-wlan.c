/*
 * arch/arm/mach-tegra/gtablet-pm-wlan.c
 *
...
/* Power control */
static void __gtablet_pm_wlan_toggle_radio(struct device *dev, unsigned int on)
{
	struct gtablet_pm_wlan_data *wlan_data = dev_get_drvdata(dev);
...
//		gpio_set_value(GTABLET_WLAN_RESET, 0); /* Assert reset */
//		gpio_set_value(GTABLET_WLAN_POWER, 0); /* Powerdown */
//		msleep(2);
		gpio_set_value(GTABLET_WLAN_POWER, 1); /* Powerup */
		msleep(2);
		gpio_set_value(GTABLET_WLAN_RESET, 1); /* Deassert reset */
		msleep(2);
...
static int gtablet_wlan_set_carddetect(struct device *dev,int val)
{
	dev_dbg(dev,"%s: %d\n", __func__, val);

	/* power module up or down based on needs */
	__gtablet_pm_wlan_toggle_radio(dev,val);
	
	/* notify the SDIO layer of the CD change */
	gtablet_wifi_set_cd(val);
	return 0;
} 

/* rfkill */
static int gtablet_wlan_set_radio_block(void *data, bool blocked)
{
	...
	return gtablet_wlan_set_carddetect(dev,!blocked);
}

static const struct rfkill_ops gtablet_wlan_rfkill_ops = {
    .set_block = gtablet_wlan_set_radio_block,
};
			...
			const char *buf, size_t count)
{
	...
	if (!strcmp(attr->attr.name, "power_on")) {
		...
		__gtablet_pm_wlan_toggle_radio(dev, on);
	} else if (!strcmp(attr->attr.name, "reset")) {
...
#ifdef CONFIG_PM
static int gtablet_wlan_suspend(struct platform_device *pdev, pm_message_t state)
{
	struct gtablet_pm_wlan_data *wlan_data = dev_get_drvdata(&pdev->dev);

	dev_dbg(&pdev->dev, "suspending\n");

	wlan_data->pre_resume_state = wlan_data->state;
	__gtablet_pm_wlan_toggle_radio(&pdev->dev, 0);

	return 0;
}

static int gtablet_wlan_resume(struct platform_device *pdev)
{
	struct gtablet_pm_wlan_data *wlan_data = dev_get_drvdata(&pdev->dev);
	dev_dbg(&pdev->dev, "resuming\n");

	__gtablet_pm_wlan_toggle_radio(&pdev->dev, wlan_data->pre_resume_state);
	return 0;
}
#else
#define gtablet_wlan_suspend	NULL
#define gtablet_wlan_resume		NULL
#endif

static struct attribute *gtablet_wlan_sysfs_entries[] = {
	&dev_attr_power_on.attr,
	&dev_attr_reset.attr,
	NULL
};

static struct attribute_group gtablet_wlan_attr_group = {
	.name	= NULL,
	.attrs	= gtablet_wlan_sysfs_entries,
};
...
static int gtablet_wlan_remove(struct platform_device *pdev)
{
	struct gtablet_pm_wlan_data *wlan_data = dev_get_drvdata(&pdev->dev);

	sysfs_remove_group(&pdev->dev.kobj, &gtablet_wlan_attr_group);
...
static struct platform_driver gtablet_wlan_driver_ops = {
	.probe		= gtablet_wlan_probe,
	.remove		= gtablet_wlan_remove,
	.suspend	= gtablet_wlan_suspend,
	.resume		= gtablet_wlan_resume,
	.driver		= {
		.name		= "gtablet-pm-wlan",
	},
};

static int __devinit gtablet_wlan_init(void)
{
	return platform_driver_register(&gtablet_wlan_driver_ops);
}

static void gtablet_wlan_exit(void)
{
	platform_driver_unregister(&gtablet_wlan_driver_ops);
}

module_init(gtablet_wlan_init);
module_exit(gtablet_wlan_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Eduardo José Tagle <ejtagle@tutopia.com>");
MODULE_DESCRIPTION("Gtablet WLAN power management");
