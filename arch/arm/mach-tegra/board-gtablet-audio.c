/*
 * arch/arm/mach-tegra/board-gtablet-audio.c
 *
...
static struct i2c_board_info __initdata gtablet_i2c_bus0_board_info[] = {
	{
...
static struct shuttle_audio_platform_data gtablet_audio_pdata = {
	.gpio_hp_det = GTABLET_HP_DETECT,
}; 

static struct platform_device gtablet_audio_device = {
	.name = "tegra-snd-gtablet",
	.id   = 0,
	.dev = {
		.platform_data = &gtablet_audio_pdata,
	}, 
};
...
static struct platform_device *gtablet_i2s_devices[] __initdata = {
	&tegra_i2s_device1,
	&tegra_i2s_device2,
	&spdif_dit_device,
	&tegra_spdif_device,
	...
	&tegra_pcm_device,
#endif
	&gtablet_audio_device, /* this must come last, as we need the DAS to be initialized to access the codec registers ! */
};

int __init gtablet_audio_register_devices(void)
{
	int ret;
	...
	ret = i2c_register_board_info(0, gtablet_i2c_bus0_board_info, 
		ARRAY_SIZE(gtablet_i2c_bus0_board_info)); 
	if (ret)
		return ret;
	return platform_add_devices(gtablet_i2s_devices, ARRAY_SIZE(gtablet_i2s_devices));
}
