...
#if defined (GTABLET_EMC_SAMSUNG)

/* Samsung: manufacturer_id = 0x0101, mem_pid = 0x5454 */
static const struct tegra_emc_table gtablet_emc_tables[] = {
	{
...
#elif defined (GTABLET_EMC_ELPIDA50NM)
...
#elif defined (GTABLET_EMC_ELPIDA40NM)
...
static const struct tegra_emc_chip gtablet_emc_chips[] = {
	{
		...
		.mem_revision_id2 = -1,
		.mem_pid = -1,
		.table = gtablet_emc_tables,
		.table_size = ARRAY_SIZE(gtablet_emc_tables),
	},
};
#endif

void __init gtablet_init_emc(void)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,38) || LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,39) 
	tegra_init_emc(gtablet_emc_chips, ARRAY_SIZE(gtablet_emc_chips));
#else
	tegra_init_emc(gtablet_emc_tables, ARRAY_SIZE(gtablet_emc_tables));
#endif
}
